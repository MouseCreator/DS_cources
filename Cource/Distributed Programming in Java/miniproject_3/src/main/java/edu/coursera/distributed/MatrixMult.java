...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        int rankOfTheProcess = mpi.MPI_Comm_rank(mpi.MPI_COMM_WORLD);
        int maxRankSize = mpi.MPI_Comm_size(mpi.MPI_COMM_WORLD);
        int nRows = c.getNRows();
        int nCols = c.getNCols();
        int rowChunk = (nRows - 1) / maxRankSize + 1;
        int startRow = rankOfTheProcess * rowChunk;
        int endRow = (rankOfTheProcess + 1) * rowChunk;
        int finalRow = Math.min(endRow, nRows);
        ...
        if (rankOfTheProcess == 0) {
            MPI.MPI_Request[] requests = new MPI.MPI_Request[maxRankSize-1];
            int rankStartRow = 0;
            int rankEndRow = rowChunk ;
            for (int i = 1; i < maxRankSize; ++i) {
                rankStartRow += rowChunk;
                rankEndRow += rowChunk;

                int rowOffset = rankStartRow * nCols;
                int elements;
                if(nRows < rankEndRow) {
                    rankEndRow = nRows;
                    elements = (rankEndRow - rankStartRow) * nCols;
                } else {
                    elements = rowChunk * nCols;
                }

                requests[i - 1] = mpi.MPI_Irecv(c.getValues(), rowOffset, elements, i ,i , mpi.MPI_COMM_WORLD);
            }
            mpi.MPI_Waitall(requests);
        } else {
            mpi.MPI_Send(c.getValues(), startRow * c.getNCols(), (finalRow - startRow) * nCols, 0, rankOfTheProcess, mpi.MPI_COMM_WORLD);
        }
    }
}
