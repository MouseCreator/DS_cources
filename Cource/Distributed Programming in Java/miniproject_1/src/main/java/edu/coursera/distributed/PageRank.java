...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
            ...
            JavaPairRDD<Integer, Double> sparkPageRand = sites.join(ranks)
                    .flatMapToPair(kv-> {
                        Integer id = kv._1();
                        ...
                        while (iterator.hasNext()) {
                            ...
                            contributions.add(new Tuple2<>(target, rank / (double) edges.getNEdges()));
                        }
                        return contributions;
                    });
            return sparkPageRand.reduceByKey(Double::sum).mapValues(v->0.15+0.85*v);
    }
}
