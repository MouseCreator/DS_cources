...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
            final Student[] studentArray) {
        return Stream.of(studentArray).parallel().filter(Student::checkIsCurrent).mapToDouble(Student::getAge).average().orElse(0);
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20Imperative(
        ...
        return (int) Stream.of(studentArray).filter(s -> s.getGrade() < 65 && s.getAge() > 20).count();
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        return (int) Stream.of(studentArray).parallel().filter(s -> s.getGrade() < 65 && s.getAge() > 20).count();
    }
}
